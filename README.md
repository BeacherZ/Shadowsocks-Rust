...
* ⚙️ **架构智能匹配**：自动精准识别 `CPU 硬件架构与系统类型`，智能下载最匹配的官方编译程序包。
* 🛠️ **一键快捷维护**：全功能`交互式菜单`，轻松搞定服务安装、版本更新、彻底卸载与配置日志查看。
* 🛡️ **定向版本检索**：检索官方源码库，逐个校验并下载个人偏好支持 `aes-128-gcm` 加密的最新版本。
* 🎯 **原生解析兼容**：默认遵循`系统 DNS` 配置，拒绝硬编码，完美保留 VPS 原生流媒体解锁能力。
